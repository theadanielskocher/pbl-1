...
\begin{itemize}
	\item Nhóm thuộc tính định danh và nhân khẩu học: Các trường dữ liệu \texttt{name}, \texttt{gender}, \texttt{birthday}, \texttt{job} được sử dụng để lưu trữ hồ sơ cơ bản. Trong đó, \texttt{gender} (Nam/Nữ) đóng vai trò là cờ điều hướng (flag) để quyết định cấu trúc phát triển của cây.
	\item Nhóm thuộc tính gia đình và tình trạng: Trường \texttt{spouseName} (Tên vợ/chồng) được gắn trực tiếp như một thuộc tính của Nút thay vì tạo một Nút độc lập, giúp giảm độ phức tạp của đồ thị cây. Trường deathDay quản lý tình trạng sinh tử, và \texttt{numChildren} (số con) được thiết kế đặc biệt chỉ kích hoạt lưu trữ đối với thành viên Nữ.
...
Để đảm bảo thuật toán chạy chính xác, cũng như bao quát được sự phức tạp của hệ thống danh xưng trong tiếng Việt, thuật toán sẽ trải qua bốn pha (phase) như sau:

\textbf{\textit{Pha thứ nhất: Truy vết đường đi về gốc (Path tracing).}}
\begin{itemize}
	...
	\begin{center}
		...
		\setlength{\tabcolsep}{4pt}
			\begin{longtable}{
				...
				\hline
				\textbf{Tọa độ} $(dA,dB)$ & \textbf{Quan hệ của B đối với A} & \textbf{Quan hệ của A đối với B} & \textbf{Giải thích Logic} \\ \hline
				\endfirsthead
				...
				$(k,0)$ & Tổ tiên đời thứ k & Hậu duệ đời thứ k & B≡L, B là gốc của nhánh chứa A. \\ \hline
				$(0,k)$ & Hậu duệ đời thứ k & Tổ tiên đời thứ k & A≡L, A là gốc của nhánh chứa B. \\ \hline
...
\begin{itemize}
	...
	\item Sau đó, hệ thống truy xuất vào thuộc tính spouseName (vợ) của nút $P$.
\end{itemize}
